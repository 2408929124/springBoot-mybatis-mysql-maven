package com.lou.springboot.config.handler;
...
public class TokenToUserMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public static byte[] getRequestPostBytes(HttpServletRequest request) throws IOException{
        int contentLent = request.getContentLength();
        if (contentLent < 0) {
            return null;
        }
        byte buffer[] = new byte[contentLent];
        for (int i = 0; i < contentLent;) {
            int readlen = request.getInputStream().read(buffer, i, contentLent - i);
            if (readlen == -1) {
                break;
            }
            i += readlen;
